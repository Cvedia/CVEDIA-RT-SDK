...
#define HANDLE_REMOTE_RESPONSE_DEFAULT HANDLE_REMOTE_RESPONSE(1, -1)
...
int securt_create_crossing_area(int const contextHandle, char const* areaId, char const* name, float const* coords, int const numPoints, int const* classes, int const classesSize, int const ignoreStationaryObjects, int areaType, double const* color)
{
	...
	LOGV << "securt_create_crossing_area called for " << context->instanceUuid.toString();
...
int securt_create_intrusion_area(int const contextHandle, char const* areaId, char const* name, float const* coords, int const numPoints, int const* classes, int const classesSize, double const* color)
{
	...
	LOGV << "securt_create_intrusion_area called for " << context->instanceUuid.toString();
...
int securt_create_loitering_area(int const contextHandle, char const* areaId, char const* name, float const* coords, int const numPoints, int const* classes, int const classesSize, int const seconds, double const* color)
{
	...
	LOGV << "securt_create_loitering_area called for " << context->instanceUuid.toString();
...
int securt_create_crowding_area(int const contextHandle, char const* areaId, char const* name, float const* coords, int const numPoints, int const* classes, int const classesSize, int const objectCnt, double const* color)
{
	...
	LOGV << "securt_create_crowding_area called for " << context->instanceUuid.toString();
...
int securt_create_line_crossing(int const contextHandle, char const* areaId, char const* name, float const* coords, int const numPoints, int const* classes, int const classesSize, int direction, double const* color)
{
	...
	LOGV << "securt_create_line_crossing called for " << context->instanceUuid.toString();
	...
	try
	{
		...
		auto const remoteLineId = utility::conversions::to_string_t(areaId);
		...
		auto const task = context->connections.secuRTLinesApi->securtCreateLineCrossingWithIdV1(remoteInstanceId, remoteLineId, lineCrossingWriteModel);
		auto const status = task.wait();
		HANDLE_REMOTE_RESPONSE_DEFAULT;
	}
	catch (std::exception const& ex)
...
int securt_create_armed_person_area(int const contextHandle, char const* areaId, char const* name, float const* coords, int numPoints, double const* color)
{
	...
	LOGV << "securt_create_armed_person_area called for " << context->instanceUuid.toString();
	...
	int numPoints, int minDuration, double const* color)
{
	...
	LOGV << "securt_create_object_left_area called for " << context->instanceUuid.toString();
	...
	try
	{
		...
		auto const model = std::make_shared<cvedia::rt::rest::client::model::AreaObjectLeftRemovedWrite>();
		model->setName(utility::conversions::to_string_t(name));
		model->setCoordinates(to_remote_coordinates(coords_to_points(coords, numPoints)));
		model->setSeconds(minDuration);
		model->setColor(to_color_vector(color));

		auto const task = context->connections.secuRTAreasApi->securtCreateObjectLeftAreaWithIdV1(remoteInstanceId, remoteAreaId, model);
		auto const status = task.wait();
		HANDLE_REMOTE_RESPONSE_DEFAULT;
}
	catch (std::exception const& ex)
	...
	int numPoints, int minDuration, double const* color)
{
	...
	LOGV << "securt_create_object_removed_area called for " << context->instanceUuid.toString();
	...
	try
	{
		...
		auto const task = context->connections.secuRTAreasApi->securtCreateObjectRemovedAreaWithIdV1(remoteInstanceId, remoteAreaId, model);
		auto const status = task.wait();
		HANDLE_REMOTE_RESPONSE_DEFAULT;
}
	catch (std::exception const& ex)
	...
	int numPoints, int const* classes, int classesSize, int maxDuration, int direction, double const* color)
{
	...
	LOGV << "securt_create_tailgating_line called for " << context->instanceUuid.toString();
	...
	try
	{
		...
		auto const model = std::make_shared<cvedia::rt::rest::client::model::LineTailgatingWrite>();
		model->setName(utility::conversions::to_string_t(name));
		model->setCoordinates(to_remote_coordinates(coords_to_points(coords, numPoints)));
		model->setClasses(to_remote_classes(classes_to_set(classes, classesSize)));
		model->setSeconds(maxDuration);
		model->setDirection(to_remote_direction(direction));
		model->setColor(to_color_vector(color));

		auto const task = context->connections.secuRTLinesApi->securtCreateLineTailgatingWithIdV1(remoteInstanceId, remoteLineId, model);
		auto const status = task.wait();
		HANDLE_REMOTE_RESPONSE_DEFAULT;
	}
	catch (std::exception const& ex)
	...
	int numPoints, double const* color)
{
	...
	LOGV << "securt_create_fallen_person_area called for " << context->instanceUuid.toString();
	...
	try
	{
		...
		auto const model = std::make_shared<cvedia::rt::rest::client::model::AreaFallenPersonWrite>();
		model->setName(utility::conversions::to_string_t(name));
		model->setCoordinates(to_remote_coordinates(coords_to_points(coords, numPoints)));
		model->setColor(to_color_vector(color));

		auto const task = context->connections.secuRTAreasApi->securtCreateFallenPersonAreaWithIdV1(remoteInstanceId, remoteAreaId, model);
		auto const status = task.wait();
		HANDLE_REMOTE_RESPONSE_DEFAULT;
	}
	catch (std::exception const& ex)
...
int securt_create_alpr_area(int const handle, char const* areaId, char const* name, float const* coords,
	int numPoints, double const* color)
{
	GET_CONTEXT(context, handle);

	LOGV << "securt_create_alpr_area called for " << context->instanceUuid.toString();
	...
	try
	{
		...
		auto const model = std::make_shared<cvedia::rt::rest::client::model::AreaAlprWrite>();
		model->setName(utility::conversions::to_string_t(name));
		model->setCoordinates(to_remote_coordinates(coords_to_points(coords, numPoints)));
		model->setColor(to_color_vector(color));

		auto const task = context->connections.secuRTAreasApi->securtCreateAlprAreaWithIdV1(remoteInstanceId, remoteAreaId, model);
		auto const status = task.wait();
		HANDLE_REMOTE_RESPONSE_DEFAULT;
	}
	catch (std::exception const& ex)
...
int securt_delete_area(int const contextHandle, char const* areaId)
{
	...
	LOGV << "securt_delete_area called for " << context->instanceUuid.toString();
...
int securt_delete_line(int const contextHandle, char const* lineId)
{
	...
	LOGV << "securt_delete_line called for " << context->instanceUuid.toString();
...
int securt_set_appearance_search(int const handle, int const mode)
{
	...
	LOGV << "securt_set_appearance_search called for " << context->instanceUuid.toString();
	...
	{
		auto const remoteInstanceId = utility::conversions::to_string_t(context->instanceUuid.toString());
		auto const req = std::make_shared<cvedia::rt::rest::client::model::SecurtSetAppearanceSearchV1_request>();
		auto remoteMode = std::make_shared<cvedia::rt::rest::client::AppearanceSearchMode>();

		switch (mode)
		{
		case 0:
			remoteMode->setValue(cvedia::rt::rest::client::AppearanceSearchMode::eAppearanceSearchMode::AppearanceSearchMode_OFF);
			break;
		case 1:
			remoteMode->setValue(cvedia::rt::rest::client::AppearanceSearchMode::eAppearanceSearchMode::AppearanceSearchMode_PERSON);
			break;
		case 2:
			remoteMode->setValue(cvedia::rt::rest::client::AppearanceSearchMode::eAppearanceSearchMode::AppearanceSearchMode_VEHICLE);
			break;
		case 3:
			remoteMode->setValue(cvedia::rt::rest::client::AppearanceSearchMode::eAppearanceSearchMode::AppearanceSearchMode_BOTH);
			break;
		default:
			remoteMode->setValue(cvedia::rt::rest::client::AppearanceSearchMode::eAppearanceSearchMode::AppearanceSearchMode_OFF);
			break;
		...
		req->setMode(remoteMode);

		auto const task = context->connections.secuRTApi->securtSetAppearanceSearchV1(remoteInstanceId, req);
		auto const status = task.wait();
		HANDLE_REMOTE_RESPONSE_DEFAULT;
	}
	catch (std::exception const& ex)
...
int securt_set_motion_area(int const contextHandle, float const* coords, int const numPoints)
{
	...
	LOGV << "securt_set_motion_area called for " << context->instanceUuid.toString();
...
int securt_set_exclusion_areas(int const contextHandle, float const* coords, int const numPoints)
{
	...
	LOGV << "securt_set_exclusion_areas called for " << context->instanceUuid.toString();
...
int securt_set_auto_restart(int const contextHandle, int const state)
{
	...
	LOGV << "securt_set_auto_restart called for " << context->instanceUuid.toString();
...
int securt_set_performance_profile(int const handle, int const profile)
{
	GET_CONTEXT(context, handle);
	...
	if (context->remoteIp.empty())
	{
		...
		return securt->setPerformanceProfile(static_cast<cvedia::rt::iface::SecuRT::PerformanceProfile>(profile)) ? 1 : -1;
	}

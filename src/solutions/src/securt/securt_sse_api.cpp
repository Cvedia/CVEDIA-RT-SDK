...
SecuRtSseApi::SecuRtSseApi(std::string const& server)
{
	server_ = server;
}

SecuRtSseApi::~SecuRtSseApi()
{
...
std::string SecuRtSseApi::securtConsumeEventsSse(std::string const& instanceId)
{
	if (eventsActive_.find(instanceId) == eventsActive_.end() || eventsActive_[instanceId] == false)
	{
		...
		eventsThread_[instanceId] = std::thread([this, instanceId]()
		{
			httplib::Client(server_)
				.Get("/v1/securt/instance/" + instanceId + "/consume_events_sse",

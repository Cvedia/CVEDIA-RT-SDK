...
#pragma GCC diagnostic ignored "-Wpragmas -Wold-style-cast"          // warning: unknown option after '#pragma GCC diagnostic' kind
...
struct ImVector
{
    ...
    inline void         reserve(int new_capacity)           { if (new_capacity <= Capacity) return; T* new_data = static_cast<T*>(IM_ALLOC((size_t)new_capacity * sizeof(T))); if (Data) { memcpy(new_data, Data, static_cast<size_t>(Size) * sizeof(T)); IM_FREE(Data); } Data = new_data; Capacity = new_capacity; }

    // NB: It is illegal to call push_back/push_front/insert with a reference pointing inside the ImVector data itself! e.g. v.push_back(v[10]) is forbidden.
    inline void         push_back(const T& v)               { if (Size == Capacity) reserve(_grow_capacity(Size + 1)); memcpy(&Data[Size], &v, sizeof(v)); Size++; }
    inline void         pop_back()                          { IM_ASSERT(Size > 0); Size--; }
    inline void         push_front(const T& v)              { if (Size == 0) push_back(v); else insert(Data, v); }
    inline T*           erase(const T* it)                  { IM_ASSERT(it >= Data && it < Data + Size); const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + 1, (static_cast<size_t>(Size) - static_cast<size_t>(off) - 1) * sizeof(T)); Size--; return Data + off; }
    inline T*           erase(const T* it, const T* it_last){ IM_ASSERT(it >= Data && it < Data + Size && it_last > it && it_last <= Data + Size); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + count, (static_cast<size_t>(Size) - static_cast<size_t>(off) - count) * sizeof(T)); Size -= static_cast<int>(count); return Data + off; }
    inline T*           erase_unsorted(const T* it)         { IM_ASSERT(it >= Data && it < Data + Size);  const ptrdiff_t off = it - Data; if (it < Data + Size - 1) memcpy(Data + off, Data + Size - 1, sizeof(T)); Size--; return Data + off; }
    inline T*           insert(const T* it, const T& v)     { IM_ASSERT(it >= Data && it <= Data + Size); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(_grow_capacity(Size + 1)); if (off < (int)Size) memmove(Data + off + 1, Data + off, (static_cast<size_t>(Size) - static_cast<size_t>(off)) * sizeof(T)); memcpy(&Data[off], &v, sizeof(v)); Size++; return Data + off; }
...
#define IM_COL32_A_MASK     0xFF000000
#endif
#define IM_COL32(R,G,B,A)    ((static_cast<ImU32>(A)<<IM_COL32_A_SHIFT) | (static_cast<ImU32>(B)<<IM_COL32_B_SHIFT) | (static_cast<ImU32>(G)<<IM_COL32_G_SHIFT) | (static_cast<ImU32>(R)<<IM_COL32_R_SHIFT))
...
struct ImColor
{
    ...
    ImColor(ImU32 rgba)                                             { float sc = 1.0f / 255.0f; Value.x = static_cast<float>((rgba >> IM_COL32_R_SHIFT) & 0xFF) * sc; Value.y = static_cast<float>((rgba >> IM_COL32_G_SHIFT) & 0xFF) * sc; Value.z = static_cast<float>((rgba >> IM_COL32_B_SHIFT) & 0xFF) * sc; Value.w = static_cast<float>((rgba >> IM_COL32_A_SHIFT) & 0xFF) * sc; }
...
struct ImDrawList
{
    ...
    inline    void  PrimVtx(const ImVec2& pos, const ImVec2& uv, ImU32 col)         { PrimWriteIdx(static_cast<ImDrawIdx>(_VtxCurrentIdx)); PrimWriteVtx(pos, uv, col); } // Write vertex with unique index

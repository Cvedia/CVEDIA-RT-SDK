...
namespace RtspUrlFetcher
{
    ...
    class RtspDemo
    {
        ...
        private static bool StartInstance(string uri)
        {
            ...
            int handle = Cvedia.Securt.CreateContext("", "");
            instances[uri] = handle;

            // create a new instance with a given ID and name
            Cvedia.Securt.CreateInstance(handle, $"instance_{index}");

            // start the instance
            Cvedia.Securt.Start(handle);

            Cvedia.Securt.UpdateInstanceOptions(handle, 1, 2, 2, 1, 10, 1, 1, 1, 0);

            // connect to the input RTSP stream
            int ret = Cvedia.Securt.SetInputToRtsp(handle, uri);
            ...
            if (Cvedia.Securt.EnableRtspOutput(handle, rtspMapping) > 0)
            {

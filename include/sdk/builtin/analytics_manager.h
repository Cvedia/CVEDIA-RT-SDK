...
namespace cvedia {
	namespace rt {
		namespace solution {
			...
			class EXPORT_CLASS AnalyticsManager
			{
			public:
				AnalyticsManager();

				EXPORT std::vector<std::unique_ptr<Event>> processMetadata(std::unique_ptr<Event> event);
//				std::vector<std::unique_ptr<Event>> processTracks(std::chrono::milliseconds frameTimestamp, rt::cvec const& tracks, rt::cbuffer image);
			...
			private:
				struct TrackHistory
				{
					...
					std::deque<std::unique_ptr<Event>> attributes;
					std::unique_ptr<Event> crop = nullptr;
					bool isMoving;
					bool causedEvent;
					int eventCount;
					long long lastSeen;
				...
				//void _updateCrop(Track const& trackData, rt::cbuffer image, std::chrono::milliseconds frameTimestamp);
				void _updateDiagnosticCrop(rt::Uuid const& uuid, rt::cbuffer image, float confidence, std::string const& label);
				//std::vector<std::unique_ptr<Crop>> _processCrops(TrackHistory & track);
				std::vector<std::unique_ptr<Event>> _maybeDelayTrack(std::unique_ptr<Event> trackEvent);
				std::vector<std::unique_ptr<Event>> _maybeDelayCrop(std::unique_ptr<Event> cropEvent);
//				std::vector<std::unique_ptr<Event>> _pruneAttribute(std::unique_ptr<Event> attributeEvent);
				std::vector<std::unique_ptr<Event>> _maybeDelayAttribute(std::unique_ptr<Event> attributeEvent);
...
//				std::vector<std::unique_ptr<Crop>> _closeLostTracks(std::unordered_set<rt::Uuid> const& seenUuids);

	...
	namespace rt {
		...
		namespace api {
			namespace inference {
				...
				NODISCARD EXPORT expected<cvec> runInference(std::string const& modelUri, cvec const& jobs);
				...
				NODISCARD EXPORT expected<std::shared_ptr<iface::InferenceEngine>> getBackendByName(std::string const& name);

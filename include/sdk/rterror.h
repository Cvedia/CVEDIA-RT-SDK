...
#define RT_CATCH_CUDA_ERROR(expr, message) \
	auto &&UNIQUE_NAME(tmp) = expr; \
	if (UNIQUE_NAME(tmp) != cudaSuccess) { \
		LOGE << message; \
		return ::cvedia::rt::unexpected(RTErrc::OperationFailed); \
	}
...
#define RT_CATCH_REST(expr, errc, errm) \
	auto &&UNIQUE_NAME(tmp) = expr; \
	if (!UNIQUE_NAME(tmp)) { \
		response.status = static_cast<int>(errc); \
		response.set_content(errm, "text/plain"); \
		return; \
	}

#define RT_TRY_REST(var, expr, errc, errm) \
	RT_CATCH_REST(expr, errc, errm) \
	var = std::move(UNIQUE_NAME(tmp).value())

#define RT_CATCH_REST_MAP(expr, errcMap, errMsgMap, defHttpCode) \
	auto &&UNIQUE_NAME(tmp) = expr; \
	auto _errc = defHttpCode; \
	std::string _errMsg; \
	if (!UNIQUE_NAME(tmp)) { \
		if (errcMap.find(UNIQUE_NAME(tmp).error()) != errcMap.cend()) { \
			_errc = errcMap.at(UNIQUE_NAME(tmp).error()); \
		} \
		if (errMsgMap.find(_errc) != errMsgMap.cend()) { \
			_errMsg = errMsgMap.at(_errc); \
		} \
		response.status = static_cast<int>(_errc); \
		response.set_content(_errMsg, "text/plain"); \
		return; \
	}

#define RT_TRY_REST_MAP(var, expr, errcMap, errMsgMap, defCode) \
	RT_CATCH_REST_MAP(expr, errcMap, errMsgMap, defCode) \
	var = std::move(UNIQUE_NAME(tmp).value())
